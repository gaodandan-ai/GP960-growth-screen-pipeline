    ...
    def load_gene_mapping(self):
        ...
        try:
            ...
            for col in mapping_df.columns:
                if 'available' in col.lower() or 'avail' in col.lower() or 'avalible' in col.lower() or '是否可用' in col:
                    available_col = col
                    break
            ...
            # 查找基因ID列 (优先选择 old_locus_tag 以匹配 01.pp_rawdata.py 的输出)
            gene_col = None
            if 'old_locus_tag' in mapping_df.columns:
            ...
            # 注意：有些 Excel 可能将 0 读成 False 或 '0'
            unavailable_mask = (mapping_df[available_col] == 0) | (mapping_df[available_col] == '0') | (mapping_df[available_col] == False)
            ...
            if source_file.exists():
                ...
                for col in columns:
                    if col.startswith('ctrol'):
                        ...
                        gene_part = col.split('-', 1)[1] if '-' in col else col
                        if gene_part not in gene_groups:
                            gene_groups[gene_part] = []
                        gene_groups[gene_part].append(col)
                    elif col == 'VC':
                        if 'VC' not in gene_groups:
                            gene_groups['VC'] = []
                        gene_groups['VC'].append(col)
                
                print(f"  发现 {len(gene_groups)} 个基因组合/控制组")
                ...
                for gene_combo, replicate_cols in gene_groups.items():
                    ...
                    # 如果是 VC，直接保留 VC 名称；否则加上 ctrol- 前缀
                    new_col_name = gene_combo if gene_combo == 'VC' else f"ctrol-{gene_combo}"
